...
#define INCOMING_FLAGS_USE_UMEM_MAKE_PRESENT    0x02
                                                  ...
                                                  QEMUFile *mig_read)
{
    ...
    if ((incoming_postcopy_flags & (INCOMING_FLAGS_FAULT_REQUEST |
                                    INCOMING_FLAGS_USE_UMEM_MAKE_PRESENT)) ==
        (INCOMING_FLAGS_FAULT_REQUEST |
         INCOMING_FLAGS_USE_UMEM_MAKE_PRESENT)) {
        fprintf(stderr, "incoming_postcopy_flags 0x%x and 0x%x "
                "can't specify simultaniously",
                INCOMING_FLAGS_FAULT_REQUEST,
                INCOMING_FLAGS_USE_UMEM_MAKE_PRESENT);
        abort();
    }
...
static int postcopy_incoming_umem_ram_load(void)
{
    ...
    if (umemd.page_cached->nr > 0) {
        ...
        if (incoming_postcopy_flags & INCOMING_FLAGS_USE_UMEM_MAKE_PRESENT) {
            umem_make_page_present(umemd.last_block_read->umem,
                                   umemd.page_cached);
        }
    }
